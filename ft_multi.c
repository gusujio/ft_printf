...
char *d[60] = {"5", "25", "125", "625", "3125", "15625", "78125", "390625", "1953125",
               ...
               "9094947017729282379150390625", "45474735088646411895751953125",
...
char *dd[60] = {"2", "4", "8", "16", "32", "64", "128", "256", "512", "1024", "2048", "4096",
...
char* ft_degr2(int k)
{
	...
	while (k > 0)
	{
		...
		{
			if (!k2[0])
				return (ft_strdup(dd[k - 1]));
			s = k2;
			k2 = ft_multi(dd[k - 1], k2);
			ft_strdel(&s);
			return (k2);
		}
		if (k >= 60 && !k2[0])
...
void ft_m2(const char *wer, const char *wer1, int *C, int r)
{
	int i;
	int k;
	int j;
...
char* ft_multi(char *wer1, char* wer)
{
	...
	if (wer[0] == 48 || wer1[0] == 48)
		return (0);
	q = ft_strlen(wer);
	ft_m2(wer, wer1, C, (r = ft_strlen(wer1)));
	i = 0;
	while (i != q + r - 1)
	{
		C[i + 1] = C[i + 1] + (C[i] / 10);
